...
$projectLinks = cocreate_project_link_rows();
...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    if (!$errors) {
        ...
        cocreate_save_project_links($pdo, $projectId, $validProjectLinks);

        if (isset($_FILES["project_image"])) {
            [$projectImage, $imageError] = save_project_image(
            ...
            if ($imageError) {
                ...
                $updateImage = $pdo->prepare(
                    "UPDATE projects SET project_image = ? WHERE project_id = ? AND user_id = ?",
                );
                $updateImage->execute([
                    $projectImage,
                    $projectId,
