...
function setupRevealMotion() {
  const targets = document.querySelectorAll('.page-head, .filter-bar, .card, .stat, .table-wrap, .quick-actions, .hero-panel, .hero-metrics > div');
...
function setupValidation() {
  ...
  document.querySelectorAll('input[type="password"][minlength]').forEach((field) => {
...
function setupLiveProjectFilters() {
  document.querySelectorAll('.filter-bar').forEach((form) => {
    ...
    const update = () => {
      ...
      const skill = (form.querySelector('[name="skill"]')?.value || '').trim().toLowerCase();
      const status = (form.querySelector('[name="status"]')?.value || '').trim();
      let visible = 0;

      cards.forEach((card) => {
        const text = card.textContent.toLowerCase();
        const escapedStatus = window.CSS && CSS.escape ? CSS.escape(status) : status.replace(/[^a-z0-9_-]/gi, '\\$&');
        const statusMatch = !status || card.querySelector(`.status-${escapedStatus}`);
